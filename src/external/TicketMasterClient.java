...
public class TicketMasterClient {
	...
	private static final String API_KEY = "dNAffnrsJC82hbadyGPQmgr1e5tq1YLC";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8"); // "Rick Sun" => "Rick%20Sun" unicode
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		//using geoHash to locate
		...
		try {
			...
			connection.setRequestMethod("GET");//声明一个method
			connection.connect();//发送request 这两行可以不写，getResponseCode 会帮你call connect
			...
			//connection.getInputStream();//返回response body
			//InputStreamReader reader = new InputStreamReader(connection.getInputStream());
			//读从互联网传来的数据 IO操作非常慢 一个一个字符读不好
			//一下都读进来也不好，内存占用量太大有风险不efficent
			//best：一段一段读 bufferedreader
		...
		try {
			...
			if (!obj.isNull("_embedded")) {//Determine if the value associated with the key is null or if there is no value
	...
	//fetch image url from event JSONObject
	...
	//fetch a set of categories from event JSONObject
	...
	//fetch address from event JSONObject
