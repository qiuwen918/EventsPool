 (function() {
  ...
  function validateSession() {
    onSessionInvalid();//show login 界面，hide其他
    // The request parameters
    var url = './login';
    var req = JSON.stringify({});//字符串化安全的JSON值  req = "{}"
    ...
    ajax('GET', url, req,
      ...
      function(res) {
        var result = JSON.parse(res);//return给定JSON字符串转换后的对象
...
//login successfully, show main page
...
//show login form，hide others
  function onSessionInvalid() {
    ...
    clearLoginError();
    showElement(loginForm);//no second argument: undefine === false, 执行displayStyle = 'block'
  }

  function hideElement(element) {
    element.style.display = 'none';//隐藏对象。与visibility属性的hidden值不同，不为被隐藏的对象保留其物理空间
  }

  function showElement(element, style) {
    var displayStyle = style ? style : 'block'; 
    element.style.display = displayStyle; //'block':元素呈现为块级元素
  }
...
//error callback for getCurrentPosition
  ...
  function register() {
    ...
    password = md5(username + md5(password));//password hash
  ...
  function activeBtn(btnId) {
    ...
    for (var i = 0; i < btns.length; i++) { //去掉btn class中的active class
      btns[i].className = btns[i].className.replace(/\bactive\b/, '');
    }

    // active the one that has id = btnId
    var btn = document.querySelector('#' + btnId);
    btn.className = 'main-nav-btn active';
  }
  ...
  function ajax(method, url, data, successCallback, errorCallback) {
    ...
    xhr.onload = function() { //onload: 当readystate = 4,会trigger function, handle request 回来
      if (xhr.status === 200) {
    ...
    xhr.onerror = function() {//onerror: request发送失败
  ...
  function changeFavoriteItem(item_id) {
    ...
    var favIcon = document.querySelector('#fav-icon-' + item_id); //<i> element(heart icon)
    var favorite = !(li.dataset.favorite === 'true');//每次点击取反
